...
struct ContentView: View {
    ...
    /// in is. Both feed FloatingCard so it opens at its button rather than in
    /// the middle of the window.
    @State private var cardButtonAnchors: [String: CGRect] = [:]
    @State private var cardSpaceSize: CGSize = .zero
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 10) {
                ...
                SlideshowActionBar(
                    ...
                    onAddPhotos: { showOnlyCard(); openPhotoPicker() },
                    onOpenMusic: { showOnlyCard(music: true) },
                    onOpenTheme: { showOnlyCard(theme: true) },
                    onOpenSettings: { showOnlyCard(settings: true) },
                    onOpenExport: { showOnlyCard(export: true) },
                    onOpenFullScreen: { showOnlyCard(); openCinemaFullScreenPreview() }
                )
            ...
            if isMusicSheetPresented {
                FloatingCard(title: "Music Playlist",
                             ...
                             anchor: cardButtonAnchors["Music"],
                             containerSize: cardSpaceSize) {
                    MusicPlaylistSheet(
                        selectedMusicURLs: selectedMusicURLs,
            ...
            if isThemeSheetPresented {
                FloatingCard(title: "Choose Theme",
                             ...
                             anchor: cardButtonAnchors["Theme"],
                             containerSize: cardSpaceSize) {
                    ThemePickerPopover(
            ...
            if isSettingsSheetPresented {
                FloatingCard(title: "Settings",
                             ...
                             anchor: cardButtonAnchors["Settings"],
                             containerSize: cardSpaceSize) {
                    LeftImportPanel(
                        timingMode: $timingMode,
                        secondsPerPhoto: $secondsPerPhoto,
            ...
            if isExportSheetPresented {
                FloatingCard(title: "Export",
                             ...
                             anchor: cardButtonAnchors["Export"],
                             containerSize: cardSpaceSize) {
                    RightExportPanel(
        ...
        // ⚠️ ONE modifier, not four. Written out inline these four lines tipped
        // this body over the type-checker's limit ("unable to type-check this
        // expression in reasonable time") - the root of this view is already
        // very large. A modifier is checked on its own and costs the body one
        // step.
        .modifier(SlideshowCardSpaceReporter(size: $cardSpaceSize,
                                             anchors: $cardButtonAnchors))
        .onAppear {
        ...
        // *„kada sam u folderu gde su slike, i kada kliknem na briefshow on
        // mora UVEK da loaduje te slike u kom sam bio folderu"*.
        //
        // initialPhotoURLs only ever reached a FRESH window; a second press
        // just refocused the one already open, so the client stood in a new
        // folder, pressed BriefShow, and the previous folder's slideshow looked
        // back at them. The window is not rebuilt to deliver them - that would
        // throw away its position and its size - the URLs are handed to the
        // view already running, down the same importPhotoURLs() path a fresh
        // window uses. Nothing new decides what counts as a photo.
        .onReceive(NotificationCenter.default.publisher(for: .briefShowLoadPhotos)) { note in
            guard let urls = note.userInfo?[BriefShowWindowController.photoURLsKey] as? [URL],
                  !urls.isEmpty else {
                return
            }
            // The same folder means the same slideshow. Re-importing would drop
            // the prepared thumbnails and reset the preview for no change on
            // screen, and pressing BriefShow twice from one folder is the
            // ordinary case, not an edge one.
            guard Self.slideshowPhotoOrder(urls) != selectedPhotoURLs else {
                return
            }
    ...
    /// ⚠️ ONE CARD AT A TIME. Requested 11.09: *„na primer sada kliknem na
    /// Theme, i posle kliknem na settings, cim kliknem na settings da se
    /// zatvori theme sto sam otvorio"*.
    ///
    /// Each card owned an independent Bool and nothing coordinated them, so
    /// opening a second one left the first sitting behind it - two draggable
    /// cards stacked over the picture, closed one at a time by hand. They are
    /// deliberately NOT sheets (see the comment where they are mounted), and a
    /// sheet is the thing that would have made them exclusive for free, so the
    /// exclusivity has to be written down somewhere. Here is that somewhere:
    /// every opener goes through this, and adding a card means adding a
    /// parameter rather than remembering to close four others.
    ///
    /// Called with no arguments it just closes everything - which is what the
    /// photo picker and the full-screen preview want before they take over.
    private func showOnlyCard(music: Bool = false,
                              theme: Bool = false,
                              settings: Bool = false,
                              export: Bool = false) {
        isMusicSheetPresented = music
        isThemeSheetPresented = theme
        isSettingsSheetPresented = settings
        isExportSheetPresented = export
    }

    /// The order the slideshow shows photos in: images only, sorted by name the
    /// way a folder reads.
    ///
    /// Pulled out of importPhotoURLs() so the "are these the photos we already
    /// have?" test below can compare like with like. Two copies of this rule
    /// would drift apart the first time one of them changed, and the symptom
    /// would be a slideshow that quietly rebuilds itself when nothing changed.
    private static func slideshowPhotoOrder(_ urls: [URL]) -> [URL] {
        urls
            .filter { url in
    ...
    private func importPhotoURLs(_ urls: [URL]) {
        let sortedURLs = Self.slideshowPhotoOrder(urls)
...
private func computeImaginationRevealTargets(
    ...
    let secondaryStartScale: CGFloat =
        usesSecondTwinScene ? 1.45 : 1.08

    let secondaryEndScale: CGFloat =
        usesSecondTwinScene ? 1.05 : 0.70
    ...
    return ImaginationRevealTargets(
        ...
        secondaryDistantStartScale: 1.56156,
        secondaryDistantEndScale: 1.20666,
            ...
            ZStack {
                ...
                if let activeImage {
                    ...
                    let secondaryCardSize: CGSize? = {
                        ...
                        let secondaryAvailableWidth = proxy.size.width * 0.72
                        let secondaryAvailableHeight = proxy.size.height * 0.76
                    ...
                    if usesTwoPhotoScene, let secondaryImage, let secondaryCardSize {
                        ...
                        ZStack {
                            Image(nsImage: secondaryImage)
                                ...
                                .scaledToFill()
                                .frame(
                                    width: secondaryCardSize.width * (usesSecondTwinScene ? 0.70 : 1.0),
                                    height: secondaryCardSize.height * (usesSecondTwinScene ? 0.70 : 1.0)
                                )
                                .clipped()
...
struct LeftImportPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            // ⚠️ NO PanelTitle here. This panel is only ever shown inside the
            // Settings card, and the card already writes "Settings / Timing and
            // transitions" in its own header. Printed twice it cost the card
            // about 50pt of height for nothing, which is exactly the height a
            // card anchored above its button does not have to spare.
...
struct ImaginationCardPage: View {
    ...
    // Druga fotografija u two-photo sceni — ista veličina kao glavna.
                ...
                if let activeImage {
                    ...
                    if usesTwoPhotoScene,
                       ...
                       let secondaryCardSize {
                        ...
                        // Drugi original: ista veličina kao glavna
                        // fotografija, samo drugačije rotiran.
                        ZStack {
                            Image(nsImage: secondaryImage)
                                .resizable()
                                .scaledToFill()
...
struct RightExportPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            // ⚠️ NO PanelTitle here — same reason as LeftImportPanel: this
            // panel lives only inside the Export card, whose header already
            // says "Export / Render your video".
            VStack(alignment: .leading, spacing: 10) {
                Text("Video Settings")
                    .font(.custom("Figtree", size: 13).weight(.medium))
                    .foregroundColor(AppColors.ink)
        ...
        .background(
            GeometryReader { geo in
                Color.clear.preference(
                    key: SlideshowButtonAnchorKey.self,
                    value: [title: geo.frame(in: .named(SlideshowCards.coordinateSpace))]
                )
            }
        )
        .onHover { hovering in
...
/// Where each action-bar button sits, in the cards' own coordinate space, so a
/// card can open at its button instead of in the middle of the window.
/// Requested 11.09: *„da se otvori uvek gde je dugme"*. Keyed by title - the bar
/// is one row and no title appears twice in it.
struct SlideshowButtonAnchorKey: PreferenceKey {
...
/// The space the cards float in. Read from a background reader rather than by
/// wrapping the root in a GeometryReader: this window's size was fought over
/// three times (see BriefShowWindowController.open) and a greedy, top-leading
/// GeometryReader at the root is exactly the kind of thing that reopened it.
/// A .background never changes the layout it sits behind.
struct SlideshowCardSpaceKey: PreferenceKey {
...
/// How tall a card's content actually is, so the card can be that tall.
/// Constants shared by the cards and the bar that opens them.
///
/// ⚠️ They live here and not on FloatingCard because that is generic over its
/// content, and Swift does not allow a static stored property on a generic
/// type. Putting them back there is a compile error, not a style question.
/// Reports the card space's size and every action-bar button's frame, and names
/// the coordinate space both are measured in.
struct SlideshowCardSpaceReporter: ViewModifier {
    @Binding var size: CGSize
    @Binding var anchors: [String: CGRect]

    func body(content: Content) -> some View {
        content
            .coordinateSpace(name: SlideshowCards.coordinateSpace)
            .background(
                GeometryReader { geo in
                    Color.clear.preference(key: SlideshowCardSpaceKey.self, value: geo.size)
                }
            )
            .onPreferenceChange(SlideshowCardSpaceKey.self) { newSize in
                size = newSize
            }
            .onPreferenceChange(SlideshowButtonAnchorKey.self) { newAnchors in
                anchors = newAnchors
            }
    }
}
...
enum SlideshowCards {
    /// The name every card and every action-bar button measures itself in.
    static let coordinateSpace = "slideshowCards"

    /// The tallest a card may get. Past this the scroller inside earns its keep;
    /// under it there is nothing to scroll and the card is as tall as it shows.
    static let maxContentHeight: CGFloat = 560
}

struct CardContentHeightKey: PreferenceKey {
...
struct FloatingCard<Content: View>: View {
    ...
    /// Where the button that opened this card is, and how big the space is.
    /// Both may be empty on the very first frame - the card simply stays
    /// centred until they arrive, which is where it used to stay for good.
    ///
    /// ⚠️ Declared BEFORE `content`. The memberwise initialiser follows
    /// declaration order, and a trailing closure has to be the last argument -
    /// put these after `content` and every call site stops compiling.
    ...
    ///
    /// ⚠️ THIS IS THE DRAG ALONE, not the card's position. Where the card opens
    /// is `basePlacement`, computed from the button and the card's real size on
    /// every pass; the two are added when the card is drawn. They used to be one
    /// stored number, and that is precisely how the card came to open ON TOP of
    /// the button it was meant to sit above: the position had to be written into
    /// state on the first frame, when the content had not been measured yet and
    /// the card was still only its header - about 100pt against the 500pt it
    /// settles at - and it stayed locked there. Measured 11.09 on Settings, and
    /// reported twice by the client with screenshots.
    ...
    /// Where this card sits when nobody has dragged it: centred over its own
    /// button and just above it, and never past the edge of the window.
    ///
    /// ⚠️ COMPUTED, NOT STORED. It is read while the card is being drawn, so it
    /// always uses the size the card actually has - no ordering between the
    /// content being measured, the frame being applied and the geometry reader
    /// reporting back can leave it holding last pass's number. Dragging is not
    /// disturbed: the drag is a separate offset added on top, and this one only
    /// changes if the card itself changes size.
    private var basePlacement: CGSize {
        guard let anchor,
              cardSize.height > 0,
        ...
        // The card is centred in the space, and the offset moves it from there.
        // It opens ABOVE its button, because the bar sits along the bottom of
        // this window and a card dropped on top of its own button hides it:
        // *„modal window da ne prekriva dugmad!"*, 11.09.
        let wantedCentreX = anchor.midX
        let wantedCentreY = anchor.minY - 12 - cardSize.height / 2

        // ...but never so far that it leaves the window. A card half off the
        // screen cannot be dragged back by a header that is off it too.
        let limitX = max(0, containerSize.width / 2 - cardSize.width / 2 - 12)
        let limitY = max(0, containerSize.height / 2 - cardSize.height / 2 - 12)

        let dx = min(max(wantedCentreX - containerSize.width / 2, -limitX), limitX)
        let dy = min(max(wantedCentreY - containerSize.height / 2, -limitY), limitY)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            .contentShape(Rectangle())
            // ⚠️ MEASURED IN THE CARD SPACE, NOT IN THE CARD. A plain
            // DragGesture() reads .local - the header's own space, and that
            // space is moved by this card's own .offset(). So while the card
            // is dragged the pointer's local position slides back by exactly
            // what the card just moved, the translation grows at half speed,
            // and the card trails the cursor at HALF the distance. Measured
            // 11.09 on the Theme card: 300 pt of mouse -> 150 pt of card, and
            // after a drag or two the cursor had slid off the header and the
            // next drag did nothing at all. The named space is on the
            // slideshow root (SlideshowCardSpaceReporter), it does not move,
            // so the translation is the true one and the card follows 1:1.
            .gesture(
                DragGesture(coordinateSpace: .named(SlideshowCards.coordinateSpace))
                    .onChanged { value in
                        dragOffset = CGSize(width: dragStart.width + value.translation.width,
            ...
            //
            // ⚠️ A ScrollView IS GREEDY. Offered 620pt it takes 620pt, whatever
            // is inside it - so the Music card, three rows and about 200pt of
            // content, stood 620pt tall with empty panel under the last track.
            // Reported 11.09 with a screenshot: *„kartica ne mora da bude
            // ovoliko velika nego onoliko koliko ima sta da se pokaze u njoj"*.
            //
            // The cap did not cause that and is not what was removed: it is
            // still here, as a ceiling rather than a height. The content is
            // measured and the scroller is given exactly that, up to the cap.
            ScrollView(.vertical) {
                content()
                    .background(
                        GeometryReader { geo in
                            Color.clear.preference(key: CardContentHeightKey.self,
                                                   value: geo.size.height)
                        }
                    )
            }
            .frame(height: min(max(contentHeight, 1), SlideshowCards.maxContentHeight))
        }
        .padding(18)
        .frame(width: width)
        .onPreferenceChange(CardContentHeightKey.self) { height in
            contentHeight = height
        }
        .background(AppColors.panel)
        .clipShape(RoundedRectangle(cornerRadius: 20))
        // ⚠️ FLATTENED BEFORE THE SHADOW, and that is about the drag, not the
        // look. *„da moze da se pomera draguje smoothly ne da secka"*. Without
        // this the shadow is recomputed from the card's whole live content -
        // panel, text, rows, every one of them - on every frame the card moves.
        // compositingGroup renders the card once into one layer and casts the
        // shadow from that, so dragging moves a finished picture.
        .compositingGroup()
        .shadow(color: .black.opacity(0.45), radius: 28, y: 10)
        // Measured AFTER padding and frame, so this is the card the client
        // actually sees - the size basePlacement needs to keep it on screen and
        // off its own button.
        .background(
            GeometryReader { geo in
                Color.clear
                    .onAppear { cardSize = geo.size }
                    .onChange(of: geo.size) { newValue in cardSize = newValue }
            }
        )
        // Where it opens, plus wherever the client has dragged it since.
...
extension Notification.Name {
    /// Photos for a BriefShow window that is already open, posted by
    /// `BriefShowWindowController.open(initialPhotoURLs:)` and received by
    /// ContentView. ContentView is built only by that controller, so this
    /// cannot reach the suite's main window.
    static let briefShowLoadPhotos = Notification.Name("briefShowLoadPhotos")
}
...
final class BriefShowWindowController {
    ...
    /// Key under which `open(initialPhotoURLs:)` hands photos to a window that
    /// is already on screen. See the post site below and ContentView's
    /// .onReceive for why this exists at all.
    static let photoURLsKey = "photoURLs"
    ...
    // ⚠️ initialPhotoURLs is used EVERY time, not only on a fresh window.
    // Until 11.09 a second press just refocused the open window and kept the
    // previous folder's photos; the client asked for the folder they are
    // standing in to win. A window that is already open is handed the URLs
    // through .briefShowLoadPhotos instead of being rebuilt, so it keeps its
    // position and size.
    //
    // ⚠️ That does mean a slideshow being built in this window IS replaced
    // when BriefShow is pressed from a DIFFERENT folder. The same-folder case
    // is filtered out on the receiving side, so the ordinary repeat press
    // costs nothing.
    func open(initialPhotoURLs: [URL] = []) {
        if let controller = windowController {
            if !initialPhotoURLs.isEmpty {
                NotificationCenter.default.post(
                    name: .briefShowLoadPhotos,
                    object: nil,
                    userInfo: [Self.photoURLsKey: initialPhotoURLs]
                )
            }
            controller.window?.makeKeyAndOrderFront(nil)
            NSApp.activate(ignoringOtherApps: true)
