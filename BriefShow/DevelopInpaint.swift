...
enum InpaintPipeline {
    ...
    /// planes of it, so it is capped rather than unbounded.
    ///
    /// ⚠️ 1024, set by the client in KORAK 112 after 2048 was tried and pulled.
    /// It is a deliberate trade of restored texture against what a saved
    /// document has to carry: at 2048 a large erase put ~3 MB into the file
    /// (measured, KORAK 109) and the client judged the result worse on screen
    /// than no restore at all. Halving the edge quarters the pixels.
    ///
    /// What it costs is that the pass no longer works at the photo's own
    /// resolution on big holes — the client's 2364px grass region is stretched
    /// 2.3x into this buffer rather than 1.15x — so less of the original grain
    /// survives to be borrowed. The measured effect is in KORAK 112.
    static let maxDetailEdge = 1024
